...
void designSpritesheet(OBJECT * obj, GLuint texture) {
    ...
    glBegin(GL_POLYGON);
    glTexCoord2f(spriteWidth * obj -> sprite.currentFigure, spriteHeight * obj -> sprite.currentLine);
    glVertex3f(obj -> position.x, obj -> position.y, 0);
    glTexCoord2f(obj -> sprite.currentFigure * spriteWidth + spriteWidth, spriteHeight * obj -> sprite.currentLine);
    glVertex3f(obj -> position.x + obj -> width, obj -> position.y, 0);
    glTexCoord2f(obj -> sprite.currentFigure * spriteWidth + spriteWidth, spriteHeight * obj -> sprite.currentLine + spriteHeight);
    glVertex3f(obj -> position.x + obj -> width, obj -> position.y + obj -> height, 0);
    glTexCoord2f(spriteWidth * obj -> sprite.currentFigure, spriteHeight * obj -> sprite.currentLine + spriteHeight);
    glVertex3f(obj -> position.x, obj -> position.y + obj -> height, 0);
    glEnd();
...
void drawBird() {
    ...
    glRotatef(bird.angle, 0.0 f, 0.0 f, 1.0 f); // Rotate the bird around the Z-axis
